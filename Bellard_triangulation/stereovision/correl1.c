...
void ImgMean(float *imgm,unsigned char *img,int size_x,int size_y)
{
	 ...
	 for(y=WY;y<size_y-WY;y++) {
			...
			a=0;
			for(j=0;j<2*WX+1;j++) a+=q[j];
			a=a*k;
			for(j=0;j<=WX;j++) n[x]=a;

			for(x=WX+1;x<size_x-WX;x++) {
				 a=n[x-1]+(q[x+WX]-q[x-WX-1])*k;
				 n[x]=a;
				 imgm[x]=(float) img[y*size_x+x] - a;
			}
	 
			for(x=size_x-WX;x<size_x;x++) n[x]=a;

			if (y==WY) {
				 for(y=0;y<=WY;y++) {
						for(x=0;x<size_x;x++) imgm[y*size_x]=(float) img[y*size_x+x] - n[x];
				 }
			} else {
				 for(x=0;x<size_x;x++) imgm[y*size_x]=(float) img[y*size_x+x] - n[x];
			}
...
void Correlate(float *imgd,uchar *img1,uchar *img2,
							 int xsize,int ysize,
							 int dmin,int dmax,
							 float threshold)
{
	 float *q_tab,*n_tab;
	 float *q,*n,*r1,*m1,*s1,*r2,*m2,*s2;
	 float a,b,c,p1,p2;
	 int i,j,x,y,d,xmin,xmax;
	 ...
	 m1= malloc(sizeof(float)*xsize);
	 s1= malloc(sizeof(float)*xsize);

	 r2= malloc(sizeof(float)*xsize);
	 m2= malloc(sizeof(float)*xsize);
	 ...
	 for(y=WY;y<(ysize-WY);y++) {
			...
			if (y==WY) {
				 ...
				 for(x=0;x<xsize;x++) {
						b=(float) img1[(y+WY)*xsize+x];
						c=(float) img1[(y-WY-1)*xsize+x];
						r1[x]+=b*b-c*c;
				 }
			}
			
			a=0;
			for(j=0;j<2*WX+1;j++) a+=r1[j];
			m1[WX]=a;
			if (a==0) s1[WX]=0; else s1[WX]=1/sqrt(a);

			for(x=WX+1;x<xsize-WX;x++) {
				 m1[x]=m1[x-1]+r1[x+WX]-r1[x-WX-1];
				 if (m1[x]==0) s1[x]=0; else s1[x]=1/sqrt(m1[x]);
			}
			...
			if (y==WY) {
				 ...
				 for(x=0;x<xsize;x++) {
						b=(float) img2[(y+WY)*xsize+x];
						c=(float) img2[(y-WY-1)*xsize+x];
						r2[x]+=b*b-c*c;
				 }
			}
			
			a=0;
			for(j=0;j<2*WX+1;j++) a+=r2[j];
			m2[WX]=a;
			if (a==0) s2[WX]=0; else s2[WX]=1/sqrt(a);

			for(x=WX+1;x<xsize-WX;x++) {
				 m2[x]=m2[x-1]+r2[x+WX]-r2[x-WX-1];
				 if (m2[x]==0) s2[x]=0; else s2[x]=1/sqrt(m2[x]);
			}
			...
			for(d=dmin;d<=dmax;d++) {
				 ...
				 if (y==WY) {
						for(x=xmin;x<=xmax;x++) {
							 a=0;
							 for(j=0;j<2*WY+1;j++) {
								 a+=(float)img1[j*xsize+x]*(float)img2[j*xsize+x+d];
							 }
							 q[x]=a;
						}
				 } else {
						for(x=xmin;x<=xmax;x++) {
							 p1=(float)img1[(y+WY)*xsize+x]*(float)img2[(y+WY)*xsize+x+d];
							 p2=(float)img1[(y-WY-1)*xsize+x]*(float)img2[(y-WY-1)*xsize+x+d];
							 q[x]+=p1-p2;
						}
				 }
				 ...
				 a=0;
				 for(j=-WX;j<=WX;j++) a+=q[xmin+j];
				 n[xmin]=a;
				 
				 for(x=xmin+1;x<=xmax;x++) {
						n[x]=n[x-1]+q[x+WX]-q[x-WX-1];
				 }
...
/*
				 for(x=xmin;x<=xmax;x++) {
						...
						for(i=-WX;i<=WX;i++) {
							 b=0;
							 for(j=-WY;j<=WY;j++) { 
									b+=(float)img1[(y+j)*xsize+(x+i)] * 
										(float)img2[(y+j)*xsize+(x+i+d)];
							 }
							 a+=b;
						}
						if (a!=n[x]) {
...
*/			
			...
			for(x=WX;x<xsize-WX;x++) {
				 ...
				 if (umax>=WX && umin<xsize-WX) {
						...
						if (umin<WX) umin=WX;
						if (umax>xsize-WX-1) umax=xsize-WX-1;
						...
						if (ucmax==umin) y0=cmax; 
						else y0=n_tab[(ucmax-x-1-dmin)*xsize+x] * s2[ucmax-1];
						
						y1=cmax;
						
						if (ucmax==umax) y2=cmax; 
						else y2=n_tab[(ucmax-x+1-dmin)*xsize+x] * s2[ucmax+1];
						
						if ( (y1*s1[x]) >= threshold ) {
							 a=(y0+y2-2*y1);
							 if ((a*s1[x])<-0.00001) {
									pmax=0.5*(y0-y2)/a;
									imgd[y*xsize+x]=(ucmax-x+pmax);
									if (!finite(imgd[y*xsize+x])) {
										 printf("Error x=%d y=%d a=%f pmax=%f y0=%f y1=%f y2=%f\n",
														x,y,a,pmax,y0,y1,y2);
									}
							 }
						}
...
void cnx_find(int x,int y)
{
	 ...
	 if (x<0 || x>=cnx_sx || y<0 || y>=cnx_sy) return; 
	 p=y*cnx_sx+x;
	 if (cnx_seen[p]>=-1 || cnx_seen[p]!=cnx_val ) return;
...
void cnx_interpolate(float *cnx_fbuf,int x,int y)
{
	 ...
	 x1=x;
	 while (x1>=0 && cnx_fbuf[y*cnx_sx+x1]==NOTFOUND) x1--; 
	 x2=x;
	 while (x2<cnx_sx && cnx_fbuf[y*cnx_sx+x2]==NOTFOUND) x2++; 
	 
	 if (x1<0 || x2>=cnx_sx) return;
						...
						float *imgd,int size_x,int size_y,int factor)
{
	 ...
	 for(k=0;k<2;k++) {
			if (k==0) rect=&rect1; else rect=&rect2;
					...
					M3 m)
{
	 ...
	 for(y=0;y<size_y1;y++) { 
			...
			for(x=0;x<size_x1;x++) {
				 x2=x2_x / w2_x;
				 y2=y2_x / w2_x;
				 if (x2>=0 && x2<size_x2 && y2>=0 && y2<size_y2) {
						x2_i=(int)(x2);
						y2_i=(int)(y2);
						buf1[y*size_x1+x]=buf2[y2_i*size_x2+x2_i];
				 } else {
						buf1[y*size_x1+x]=0;
				 }
				 x2_x+=m.m[0][0];
...
int main(int argc,char *argv[])
{
	 ...
	 V3 *pts,center;
	 int xsize[2],ysize[2];
	 unsigned char *img_org[2],*img_rect[2];
	 float threshold;
	 ...
	 if (rect_given) {
			...
			for(i=0;i<2;i++) {
				 ...
				 if (rect_save) {
						sprintf(buf,"rect%d.pgm",i+1);
						PGM_Save(img_rect[i],size_x,size_y,buf);
						exit(0);
				 }
			}
	 } else {
			for(i=0;i<2;i++) img_rect[i]=img_org[i];
			rect[0]=M3_Identity();
			rect[1]=M3_Identity();
	 }

	 
	 imgd=calloc(size_x*size_y,sizeof(float));
	 pts=malloc(size_x*size_y/(factor*factor)*sizeof(V3));

	 ImgMean(imgd,img_rect[0],size_x,size_y);
	 
	 Correlate(imgd,img_rect[0],img_rect[1],size_x,size_y,
						 dmin,dmax,threshold);
	 ...
	 display(pts,center,size_x/factor,size_y/factor,window_x,window_y,mapping);
